...
char** LoadWords(bool readFromAnswersList) { //load words from string table in resource file
    char** listOfWords = NULL;
    WCHAR tempBuffer[6] = { 0 };
    DWORD tempBufferSize = sizeof tempBuffer;
    char buffer[6] = { 0 };
    DWORD stringID = 0;
    
    if (readFromAnswersList) { //list of possible answers and list of valid words differ but are stored in same table
        stringID = 101;
    ...
    if (readFromAnswersList) {
        while (index < NUMBER_OF_ANSWER_WORDS && stringID < 2410) { //2410 is the ID of start of the valid words list
            LoadString(GetModuleHandle(NULL), stringID, tempBuffer, tempBufferSize);

            wcstombs(buffer, tempBuffer, sizeof tempBuffer);
    ...
    if (!readFromAnswersList) {
        while (index < NUMBER_OF_VALID_WORDS && stringID < 17264) { //17264 is the last ID of string table
            LoadString(GetModuleHandle(NULL), stringID, tempBuffer, tempBufferSize);
