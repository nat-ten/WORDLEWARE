...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        listOfAnswerWords = LoadValidWords(fileToOpen, readFromValidAnswersList); //list of valid words differ greatly from list of words that can be answers
        randomWord = GetRandomWord(listOfAnswerWords);

        readFromValidAnswersList = FALSE;
        listOfValidWords = LoadValidWords(fileToOpen, readFromValidAnswersList);
